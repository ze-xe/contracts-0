...
contract Helper is Multicall {
    ...
    function tradingBalanceOf(address vault, address[] memory tokens, address user) public view returns(uint[] memory balance){
        uint[] memory returnData = new uint[](tokens.length);
        for(uint256 i = 0; i < tokens.length; i++) {
            returnData[i] = Vault(vault).userTokenBalance(user, tokens[i]);
        }
        return returnData;
    }
}